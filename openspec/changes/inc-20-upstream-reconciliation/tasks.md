...
- [x] 2.1 [GREEN] `internal/cli/sync.go:509`: sustituir `filepath.Join(homeDir, ".gentle-ai", "backups")` por `backup.BackupRootFor(homeDir)`.
- [x] 2.2 [GREEN] `internal/cli/run.go:709`: mismo cambio.
- [x] 2.3 [GREEN] `internal/update/upgrade/executor.go:488` (snapshot) y `:514` (poda): mismo cambio en ambos sitios. Nota explícita [D-12]: la poda (`backup.Prune`) pasa a operar **solo** sobre la raíz canónica; la raíz heredada deja de podarse (queda congelada, nunca borrada por retención — comportamiento deliberado, no un olvido).
- [x] 2.4 [GREEN] `internal/components/uninstall/service.go:178` (+ `os.MkdirAll` en `:179`, sin cambios): mismo cambio. `cleaners.go` del mismo paquete permanece intocado (V2, ruta prohibida por fichero).
- [x] 2.5 [GREEN] `internal/dashboard/service.go:1190` (+ `os.MkdirAll` en `:1191`, sin cambios): mismo cambio — este sitio ya escribía la grafía correcta (E3), el arreglo es de método, no de valor.
- [x] 2.6 [GREEN] `internal/app/app.go:1097-1100`: sustituir los dos literales del slice `roots` por `backup.BackupRoots(homeDir)`, preservando el orden vigente-primero.
- [x] 2.7 [GREEN] `cmd/axiom/main.go:141`: cambiar `"No hay respaldos registrados en ~/.axiom/backups/"` por un mensaje que nombre las **dos** raíces que `ListBackups` escanea tras 2.6 (canónica y heredada).
- [x] 2.8 [GREEN] Reapuntar las 16 aserciones que siembran o leen contra el literal heredado, verificándolas contra `backup.BackupRootFor(...)`/`backup.LegacyBackupRootFor(...)` en vez de contra una cadena re-tecleada: `internal/app/app_test.go:39,87,151,498`; `internal/cli/dedup_prune_test.go:20,90,158,226`; `internal/cli/backup_metadata_test.go:35,92,139`; `internal/update/upgrade/effective_method_routing_test.go:258,403`; `internal/update/upgrade/executor_test.go:893`; `internal/cli/restore_test.go:19`; `internal/cli/sync_test.go:1867`. Verificación: `go test ./internal/app/... ./internal/cli/... ./internal/update/upgrade/... -run 'TestApp|TestDedupPrune|TestBackupMetadata|TestEffectiveMethodRouting|TestExecutor|TestRestore|TestSync' -v` — cada uno debe seguir en verde tras el reapuntado (verifica ahora contra el accesor, no contra una copia del literal).

  > **Ajuste de entrega (2026-09-19, aplicado en `sdd-apply`, Fase 2).** El patrón de verificación de esta tarea (`-run 'TestApp|TestDedupPrune|...'`) no coincide con ninguna función real: `dedup_prune_test.go`/`backup_metadata_test.go` definen `TestPrepareBackupStep_*`, no `TestDedupPrune*`/`TestBackupMetadata*`; el sitio de `executor_test.go:893` vive dentro de `TestExecute_UpgradeBackupManifestHasUpgradeMetadata`, no de un `TestExecutor*`. Se verificó con los nombres reales (`TestPrepareBackupStep`, `TestExecute_UpgradeBackupManifestHasUpgradeMetadata`, etc.), no con el patrón literal de la tarea. Además, de los 16 sitios, **14 usan `LegacyBackupRootFor`** (fixtures autocontenidos que no dependen de ningún escritor migrado: siembran o pasan la raíz explícitamente) y **2 exigen `BackupRootFor`** (`executor_test.go:893` y `internal/cli/sync_test.go:1867`), porque ambos leen el directorio donde un escritor migrado (`Execute`, `newSyncRuntime`) escribe de verdad tras las tareas 2.1–2.3; dejarlos en `LegacyBackupRootFor` los habría roto de verdad (confirmado: `TestExecute_UpgradeBackupManifestHasUpgradeMetadata` falló con `ReadDir backups: … no encuentra la ruta` antes del reapuntado).

- [x] 2.9 [RED — control negativo explícito, antes de tocar el test] En `internal/cli/compatibility_skills_test.go`, junto a la aserción de `:131-133`, sembrar deliberadamente un respaldo bajo la raíz **canónica** (`backup.BackupRootFor(home)`) dentro del mismo subtest y confirmar que, con la aserción todavía apuntando a la raíz heredada, el test **no lo detecta** (evidencia de que hoy vigila un directorio que nadie va a crear).
- [x] 2.10 [GREEN] Reapuntar `compatibility_skills_test.go:131-133` a la raíz canónica (`backup.BackupRootFor(home)`) y confirmar que el control negativo de 2.9, ejecutado de nuevo contra la aserción ya reapuntada, **falla como se espera** (el test vuelve a ser capaz de fallar por el motivo para el que se escribió).
- [x] 2.11 [RED — control negativo explícito] Repetir 2.9 para `internal/cli/compatibility_transaction_windows_test.go:275-277`.
- [x] 2.12 [GREEN] Repetir 2.10 para `compatibility_transaction_windows_test.go:275-277`.

  > **Ajuste de entrega (2026-09-19).** La siembra deliberada de 2.9/2.11 (`os.MkdirAll(backup.BackupRootFor(home), ...)`) se insertó, se ejecutó dos veces (rojo con la aserción vieja, rojo-detectado con la aserción nueva) y **se retiró** antes de dejar el fichero en verde — dejarla en el árbol habría hecho fallar el test permanentemente (siembra una raíz que la propia aserción declara indebida). El código final solo conserva la aserción reapuntada a `backup.BackupRootFor(...)` y un comentario que documenta el control negativo ya ejecutado, no el código de siembra en sí. Las cuatro ejecuciones reales (rojo ciego → rojo detectado → verde limpio, por cada uno de los dos ficheros) están transcritas en el informe de esta fase.
...
- [x] 2.13 [Integración] Escribir/extender un test de integración que siembre `~/.gentle-ai/backups/<id>` con su manifiesto sobre un `HOME` temporal y confirme que `app.ListBackups()`, la restauración y `backup.DeleteBackup` lo siguen resolviendo, restaurando y borrando sin cambios — el contrato de lectura dual de INC-12 permanece intacto [D-12].
  ...
  > **Ajuste de entrega (2026-09-19).** Se extendió `TestRunArgsRestoreByIDWithYes` (`internal/app/app_test.go`, ya reapuntada en 2.8) en vez de crear un test nuevo: ya sembraba un respaldo bajo la raíz heredada y ejercitaba `RunArgs(["restore", ...])` de punta a punta (que despacha a `internal/cli.RunRestore`, el mismo paquete que 2.1 migra). Se añadieron dos bloques: antes de restaurar, `ListBackups()` debe listar el ID sembrado; después de restaurar, `backup.DeleteBackup(m)` debe borrar el directorio. Un test aparte habría duplicado casi todo el fixture ya existente.

- [x] 2.14 [REFACTOR] Revisar los 5 escritores y el lector: sin literales `.gentle-ai`/`.axiom` remanentes fuera de `internal/backup` (confirmarlo leyendo la salida de la tarea 2.15 antes de darla por hecha).
- [x] 2.15 [Verificación de cierre F0.b] Ejecutar `go test ./cmd/axiom/... -run TestUserStateRootsResolveThroughOwningPackage -v` (debe estar ahora en **verde**, cero violaciones). Ejecutar `go build ./...`, `go vet ./...`, `go test ./internal/... -timeout 900s`. Ejecutar `gofmt -l` solo sobre los ficheros tocados en las tareas 2.1–2.13. Confirmar que el diff no toca ninguna ruta prohibida.

  > **Ajuste de entrega (2026-09-19).** `go test ./internal/...` sin filtrar no se ejecutó completo en Windows: `internal/cli` agota los 900s por el cuelgue preexistente ya documentado por la Fase 1 (`TestCommittedCorrectionStatusAfterCommitAnswersTheTransitionItBinds`, ajeno a esta fase). En su lugar: (a) `go build ./...` y `go vet ./...` sin filtrar, verde en Windows; (b) suite completa sin `-run` en Windows para `internal/backup`, `internal/app`, `internal/update/upgrade`, `internal/components/uninstall`, `internal/dashboard`, `cmd/axiom` — todo verde; (c) `internal/cli` verificado con ejecuciones dirigidas por nombre real cubriendo cada sitio tocado (§2.8/2.9-2.12/2.13) más los paquetes de compatibilidad y sync — todo verde; (d) `docker exec axiom-bench` (contenedor provisto por el coordinador) para `go build ./...`, `go vet ./...` y `go test ./internal/backup/... ./internal/cli/... ./cmd/axiom/...` sin filtrar, incluida la suite completa de `internal/cli` en Linux. Salida literal de ambos entornos en el informe de esta fase.
...
- [x] 3.1 [GREEN] Modificar `.goreleaser.yaml`: `project_name: gentle-ai` (`:3`) → `axiom`; sustituir el build único (`:12-29`) por dos builds nombrados — `id: axiom, main: ./cmd/axiom, binary: axiom` y `id: gentle-ai-deprecated, main: ./cmd/gentle-ai, binary: gentle-ai` — preservando `env`, `goos`, `goarch`, `flags` y `ldflags` (`:25-29`, incluida la referencia a la ruta de módulo, que F1 reescribirá después) en ambos.
- [x] 3.2 [RED→GREEN] Añadir a `canonical_binary_test.go`: `TestReleaseArtifactBuildsCanonicalBinary` — parsea `.goreleaser.yaml` y confirma que `builds[]` contiene una entrada con `main: ./cmd/axiom` y `binary: axiom`. RED antes de 3.1 (falla, solo existe `./cmd/gentle-ai`), GREEN después. **Hallazgo no previsto:** `internal/releasepolicy/policy.go` fija un golden YAML propio que `TestReleaseDistributionPolicyAssertionFailsClosed` compara estructuralmente contra `.goreleaser.yaml` (verde antes de tocar nada); se actualizó ese golden en el mismo commit para reflejar los dos `builds[]`; de lo contrario, ese test — ajeno a las fuentes de alcance de la fase — habría quedado en rojo. Reconfirmado en verde después.
- [x] 3.3 [GREEN] `.github/workflows/ci.yml:266`: `go build ... -o "$env:RUNNER_TEMP\gentle-ai.exe" ./cmd/gentle-ai` → `./cmd/axiom` (renombrar también la variable de ruta si el resto del step la referencia por nombre). No se renombró: nada en `.go` lee `GENTLE_AI_TEST_BINARY`, y es el mismo convenio que `ci.yml` ya usa en `:85`/`:144` (cambiar el objetivo, no el nombre del artefacto).
- [x] 3.4 [RED→GREEN] Añadir `TestWorkflowsBuildCanonicalBinary`: recorre `.github/workflows/*.yml` y rechaza cualquier objetivo de construcción `./cmd/gentle-ai` que no figure en una lista de excepciones con motivo escrito. RED antes de 3.3 (señala `:266`), GREEN después con la lista de excepciones **vacía** (el audit de esta fase no encontró ningún caso legítimo de construir el shim).
- [x] 3.5 [Caracterización] Añadir `TestAppDispatchIsSubsetOfCanonicalDispatch`: compara con `go/ast` los verbos de `internal/app/app.go` (`:84-145`, `(read-only)`) contra los de `cmd/axiom/main.go` como conjuntos. Ejecutar y confirmar que **ya está en verde** (los 4 verbos que faltaban — `codegraph`, `telemetry`, `skill-registry`, `bench-model-picker` — ya se corrigieron antes de este incremento, `main.go:117-120,391-395` `(read-only)`); registrar esta confirmación como evidencia, no asumirla.
- [x] 3.6 [Caracterización] Añadir `TestDeadcodeRatchetTargetsCanonicalBinary`: lee `scripts/deadcode-ratchet.sh` y confirma `DEADCODE_TARGET:-./cmd/axiom` (`:41`, `(read-only)`). Ejecutar y confirmar que **ya está en verde**; registrar la confirmación.
- [x] 3.7 [GREEN] Añadir a `.github/workflows/ci.yml` el paso nuevo "Smoke the canonical binary surface": construye `./cmd/axiom` y ejercita, para cada verbo que el contrato de nombre toca, su forma de ayuda o sin argumentos, afirmando (a) el código de salida esperado y (b) que stderr **no** contiene el aviso de deprecación de `cmd/gentle-ai/main.go:14,17` `(read-only)`. Cubre como mínimo `init`, `change`, `workspace`, `handoff`, `role`, `skill`, `semantic`, `archive`, `ui` (bloqueante) y, mientras exista, `odd` (bloqueante hasta la Fase 11).
  > **Hallazgo empírico (probado en un directorio aislado, nunca el checkout):** `init`/`ui` sin argumentos NO son inertes — `init` escribe `axiom.yaml` y estructura real; `ui` arranca un servidor HTTP que bloquea. Por eso usan `--help`; los otros 9 (incluido `project`, verificado igual: exit 1, stderr limpio, sin mutación) sí son seguros sin argumentos. `git status --porcelain` final añadido como guarda independiente. **`project` añadido:** REQ-20.15 lo exige en su suelo ("al menos"); esta tarea lo omitía — el spec manda, ya cubierto. **D-07 diferido, no silenciado:** `codegraph`/`telemetry`/`skill-registry`/`bench-model-picker` (nombrados por design.md D-07, fuera del suelo de REQ-20.15) quedan informativos en la ventana de la tarea 3.8, asumidos por INC-21.
- [x] 3.8 [GREEN] En el mismo paso, añadir el bloque de comentario de ventana informativa fechada (contrato de REQ-20.16): fecha de apertura, inventario de la superficie no ejercitada (se rellena cuando el paso corra por primera vez y destape algo, no antes — O-4), e incremento sucesor nombrado **INC-21**. El resto de la superficie exclusiva de `cmd/axiom` (más allá de los verbos de la tarea 3.7) queda informativa, nunca bloqueante, hasta que INC-21 la asuma.
- [x] 3.9 [REFACTOR] Revisar `canonical_binary_test.go`: las 5 aserciones (4 de esta fase + la de la Fase 1) comparten helpers sin duplicar el parseo de `go/ast`. Aplicado por construcción desde la primera aserción de esta fase, no como pasada separada: `repositoryRoot` (nuevo) sustituye la resolución de raíz de repositorio que la Fase 1 dejó en línea, y `canonicalDispatchVerbs` reutiliza `stringLiteralValue` (ya existente) en vez de reimplementar la extracción de literales de `go/ast`.
- [x] 3.10 [Verificación de cierre F0.c1] Ejecutar `go build ./...`, `go vet ./...`, `go test ./cmd/axiom/... -v` (las 5 aserciones en verde). Build local de `./cmd/axiom` y `./cmd/gentle-ai`; confirmar por inspección de stderr que solo el segundo emite el aviso de deprecación. `gofmt -l` sobre los ficheros tocados. Confirmar que el diff no toca ninguna ruta prohibida. **Evidencia:** todo lo anterior verde/confirmado; detalle completo en el informe de aplicación de esta fase.
...
- [x] 4.1 [Punto de control condicional — REQ-20.8, decisión de mantenedor, no de código] Antes de tocar `install.sh`/`install.ps1`/las 3 compuertas de release: confirmar si ya existe un artefacto de release de Axiom publicado bajo su propia identidad de distribución (tras el cambio de `.goreleaser.yaml` de la Fase 3). **Si NO existe todavía:** detener aquí las tareas 4.2–4.6; dejarlas como una fase F0.c2b posterior, a ejecutar en cuanto exista un release publicado; continuar con las tareas 4.7 en adelante (crosslane y telemetría), que son incondicionales. **Si SÍ existe:** continuar con 4.2.

  > **Ajuste de entrega (2026-09-19).** Compuerta evaluada por el orquestador antes de esta ejecución de `sdd-apply`, con evidencia medida y no en silencio: `gh release list --repo IGutierrezZ/axiom` → **vacío, cero releases publicados** bajo la identidad de distribución de Axiom. Los tags presentes en el remoto (`v1.0.0`, `v1.0.1`, `V1.25.3`, `advisory`, …) son heredados del historial de upstream, no artefactos publicados bajo la identidad de Axiom, y por tanto no satisfacen la condición de REQ-20.8. **Resultado: NO existe todavía → rama "detener aquí" tomada.** Las tareas 4.2–4.6 quedan diferidas a **F0.c2b**, a ejecutar en cuanto exista un release publicado; se continuó con 4.7 en adelante.
- [ ] 4.2 [GREEN — condicionada a 4.1, DIFERIDA a F0.c2b] `scripts/install.sh:17-21`: `GITHUB_OWNER`, `GITHUB_REPO`, `BINARY_NAME`, `BREW_TAP`, `BREW_FORMULA_REF` → identidad de Axiom; actualizar también la cabecera de uso (`:5-14`).
- [ ] 4.3 [GREEN — condicionada a 4.1, DIFERIDA a F0.c2b] `scripts/install.ps1`: equivalentes de las mismas variables.
- [ ] 4.4 [GREEN — condicionada a 4.1, DIFERIDA a F0.c2b] `.goreleaser.yaml`: bloque `brews:` (`:123-133`) — `name: gentle-ai` → `axiom`, `homepage`, `description` según la identidad de Axiom, conservando `owner`/`repository` si el tap sigue siendo el mismo.
- [ ] 4.5 [GREEN — condicionada a 4.1, DIFERIDA a F0.c2b] `scripts/verify-release-assets.sh:11,84`, `scripts/release-preflight.sh:18`, `scripts/promote-stable-preflight.sh:18`: literal `Gentleman-Programming/gentle-ai` → identidad de Axiom en la aserción de `$GITHUB_REPOSITORY`.
- [x] 4.6 [Verificación del tramo condicional] Si 4.2–4.5 se ejecutaron: confirmar por lectura que el instalador resultante instalaría el binario `axiom`, no el de upstream (D2.1). Si se difirieron por 4.1: dejarlo anotado explícitamente en el informe de esta fase, no en silencio.

  > **Ajuste de entrega (2026-09-19).** 4.2–4.5 se difirieron (ver anotación de 4.1). Constancia explícita, no en silencio: `scripts/install.sh`, `scripts/install.ps1`, `.goreleaser.yaml` (bloque `brews:`), `scripts/verify-release-assets.sh`, `scripts/release-preflight.sh` y `scripts/promote-stable-preflight.sh` **no se tocaron** en esta fase. `scripts/install.sh` sigue citando la identidad de upstream hasta que exista un release de Axiom publicado (REQ-20.8, escenario "El instalador no se repunta sin releases propios"). Reejecutar el punto de control 4.1 en **F0.c2b** en cuanto `gh release list --repo IGutierrezZ/axiom` deje de estar vacío.
- [x] 4.7 [GREEN] `scripts/crosslane/battery.go:274`: `words[0] != "gentle-ai"` → tolerancia dual (`words[0] != "axiom" && words[0] != "gentle-ai"`), vigente primero en cualquier lógica de prioridad.
- [x] 4.8 [GREEN] `scripts/crosslane/host.go:119`: mismo cambio.
- [x] 4.9 [GREEN] `scripts/crosslane/hostopencode.go:231-243`: el shim ejecutable en `$PATH` resuelve ambos nombres mientras D2.4 mantenga el alias `gentle-ai`.

  > **Ajuste de entrega (2026-09-19).** Regla 10 de «Reglas de Comprobación y Alcance» fija `strict_tdd: true` para **todas** las fases salvo la 17; esta redacción de 4.7–4.9 no llevaba marcas `[RED]`/`[GREEN]` explícitas como sí las llevan las Fases 1–3. Se resolvió a favor de la regla general (aplica a toda fase no exceptuada) y se siguió TDD real: tres tests nuevos en `scripts/crosslane/battery_test.go` (`TestRunCommandLineAcceptsAxiomAndGentleAI`, `TestRunCommandLineEnvAcceptsAxiomAndGentleAI`, `TestHostShimPathResolvesAxiomAndGentleAI`) capturados en **rojo** contra el código sin modificar (el caso `axiom` fallaba con `unexpected provider command`; el shim de `hostopencode.go` no creaba el fichero `axiom`), y en **verde** tras 4.7–4.9. Evidencia real en el informe de esta fase.
- [x] 4.10 [GREEN] `deploy/telemetry/gentle-telemetry.service`, `deploy/telemetry/gentle-telemetry-backup.service`, `deploy/telemetry/gentle-telemetry-backup.timer`, `deploy/telemetry/gentle-telemetry-backup.test.sh`, `deploy/telemetry/install.sh` y los paneles de Grafana asociados (si existen bajo el mismo directorio): nombres de servicio → identidad de distribución de Axiom (REQ-20.12). Confirmar primero con `fd . deploy/telemetry` el inventario real antes de editar, ya que este documento no reenumera su contenido interno.

  > **Ajuste de entrega (2026-09-19).** Inventario real confirmado (`find deploy/telemetry -type f`): además de los 5 ficheros ya nombrados, existen `apache/telemetry-vhost.conf.tmpl`, `grafana/dashboards/gentle-ai-usage.json`, `grafana/provisioning/{dashboards,datasources}/telemetry.yaml`. Renombrados con toda referencia cruzada seguida (`Requisite=`, `ExecStart=` del script de respaldo, mapeo implícito `.timer`→`.service` por nombre base, `docs/telemetry-collector.md`): `axiom-telemetry.service`, `axiom-telemetry-backup`+`.service`+`.timer`+`.test.sh`; la prosa de cabecera de `apache/telemetry-vhost.conf.tmpl:1` (nombra el subdominio, que `install.sh` renderiza desde `__DOMAIN__`, asi que no es un nombre DNS literal); en Grafana, folder ("Axiom"), nombre de provider/datasource y `admin_user` (axiom); título/tags/4 descripciones del dashboard. **Límite deliberado, no en silencio:** el binario del colector (`ExecStart=/usr/local/bin/gentle-telemetry`, ligado a `cmd/gentle-telemetry`, Go fuera de alcance de esta fase), el usuario/grupo de sistema `gentle-telemetry`, `StateDirectory`/`CONFIG_DIR`/rutas de log y ACME, el `uid` interno del datasource de Grafana (`gentle-telemetry-sqlite`, 44 referencias cruzadas en el dashboard) el nombre de fichero del dashboard JSON, y en `apache/telemetry-vhost.conf.tmpl` tanto las rutas de maquina (`/var/www/gentle-telemetry-acme`, `/var/log/gentle-telemetry/`, el `LogFormat`) como el comentario de `:59`, que nombra el binario del colector y no el servicio, permanecen sin cambio — motivo escrito en los propios ficheros (`axiom-telemetry.service`, `axiom-telemetry-backup.service`). **Riesgo operativo detectado y NO resuelto por esta fase, según instrucción explícita de reportarlo en vez de decidirlo:** un host que ya tuviera este kit desplegado con los nombres antiguos quedaría con `gentle-telemetry.service`/`gentle-telemetry-backup.timer` activos y sin ruta de migración codificada al ejecutar el `install.sh` actualizado — instalaría las unidades nuevas sin detener ni retirar las viejas, con riesgo de colector duplicado compitiendo por el puerto `127.0.0.1:18181`. Es una decisión de operaciones (cuándo y cómo migrar un host ya desplegado), no de código; ver el informe de esta fase. **Superficie de test que destapó el CI, no la verificación local:** `internal/telemetrycollector/runtime_dashboard_test.go` fija la identidad del dashboard de Grafana, y este renombrado movió tres de las cadenas que asserta — el `title`, el título de fila «Where Gentle AI runs» (cuatro ocurrencias) y la descripción del panel 551, seis líneas en total. Sus aserciones del `uid` del dashboard y del `uid` del datasource siguen válidas y sin tocar: son los contratos de máquina que esta fase conserva a propósito. Las descripciones de los paneles 302 y 304, también renombradas, no las fija ningún test.
- [x] 4.11 [Verificación de cierre F0.c2] `go build ./...`, `go vet ./...` (ningún paquete Go cambia en esta fase, deben seguir en verde sin más). Lectura dirigida de cada fichero modificado para confirmar el literal correcto. `gofmt -l` sobre cualquier `.go` tocado (ninguno esperado). Confirmar que el diff no toca `contracts/**` (REQ-20.10, byte a byte) ni ninguna ruta prohibida.

  > **Ajuste de entrega (2026-09-19).** La premisa "ningún paquete Go cambia en esta fase" / "`gofmt -l` … ninguno esperado" no se sostiene contra las propias tareas 4.7–4.9 de esta misma fase, que exigen editar `scripts/crosslane/{battery,host,hostopencode}.go` (paquete Go) por REQ-20.11. Se aplicó el criterio correcto en vez del literal: `go build ./...` y `go vet ./...` en verde sin filtrar, y `gofmt -l` sobre los `.go` efectivamente tocados (los 3 de crosslane + su test) limpio — la barra de calidad que la tarea pedía para "ningún .go", aplicada honestamente a los que sí cambiaron. `git diff main...HEAD --stat` final de la Fase 4: 14 ficheros, 124 inserciones + 61 borrados = 185 líneas. `contracts/**` y toda ruta prohibida: cero coincidencias en `git diff main...HEAD --name-only`.
