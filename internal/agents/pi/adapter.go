...
// gentlePiPackageSource returns the selected gentle-pi package source read-only.
// It never calls mergePiSettingsFile and respects PI_CODING_AGENT_DIR via
// CodeGraphPaths/AgentConfigPath. Project settings take precedence over user.
func gentlePiPackageSource(cwd, agentDir string) string {
	// Reuse the resolver's selection via effective agent dir; kept thin to stay read-only.
	// Inline to avoid import cycle - mirrors selectedPackageSource without extra deps.
	agent := strings.TrimSpace(agentDir)
	if agent == "" {
		agent = strings.TrimSpace(os.Getenv("PI_CODING_AGENT_DIR"))
		if agent == "" {
			if home, err := os.UserHomeDir(); err == nil && home != "" {
				agent = AgentConfigPath(home)
			}
		}
	}
	readSource := func(path string) string {
		...
		for _, p := range piPackagesAsSlice(obj["packages"]) {
			if s, ok := p.(string); ok && (strings.HasPrefix(s, "npm:") || strings.HasPrefix(s, "git:") || strings.HasPrefix(s, "local:")) {
				return s
			}
		}
		return ""
	...
	for _, p := range []string{filepath.Join(cwd, ".pi", "agent", "settings.json"), filepath.Join(cwd, ".pi", "settings.json")} {
		if src := readSource(p); src != "" {
			return src
		}
	}
	if agent != "" {
		if src := readSource(filepath.Join(agent, "settings.json")); src != "" {
			return src
		}
	}
	return ""
