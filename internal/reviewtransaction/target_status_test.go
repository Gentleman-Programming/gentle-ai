...
func TestCanonicalRecoveryCandidateDominatesNoncanonicalAmbiguityCandidate(t *testing.T) {
	repo, base, canonical, _, _ := approvedBaseDiffScopeRecoveryFixture(t, "canonical-recovery")
	noncanonical := canonical
	noncanonical.LineageID = "noncanonical-ambiguity"
	store := writeTerminalTargetStatusAuthority(t, repo, noncanonical)
	receipt, err := os.ReadFile(store.ReceiptPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || status.Applicability != TargetApplicabilityCurrent || status.LineageID != canonical.LineageID ||
		status.Action != TargetStatusActionRecover || status.ActionDisposition != RecoveryScopeChanged || len(status.CandidateLineageIDs) != 0 {
		t.Fatalf("STATUS = %#v, %v", status, err)
	}
}

func TestExactClaimantAndCanonicalRecoveryCandidateStayAmbiguous(t *testing.T) {
	repo, base, recovery, _, _ := approvedBaseDiffScopeRecoveryFixture(t, "canonical-recovery")
	claimantPredecessor := recovery
	claimantPredecessor.LineageID = "claimant-predecessor"
	claimantStore := writeTerminalTargetStatusAuthority(t, repo, claimantPredecessor)
	claimantRecord, err := claimantStore.Load()
	if err != nil {
		t.Fatal(err)
	}
	stageStagedScopeExtra(t, repo)
	...
	claimant := newCompactStateForStagedScopeRecovery(t, repo, claimantPredecessor, snapshot, "exact-claimant")
	request := CompactRecoveryRequest{
		PredecessorLineageID: claimantPredecessor.LineageID, ExpectedPredecessorRevision: claimantRecord.Revision,
		Successor: claimant, Disposition: RecoveryScopeChanged, Actor: "maintainer", Reason: "claim expanded scope",
	}
	request.MaintainerAuthorization = compactApprovedStagedScopeRecoveryAuthorizationBinding(
		...
		claimant.LineageID, request.Actor, request.Reason,
	)
	...
	if err != nil || status.Applicability != TargetApplicabilityAmbiguous || status.Action != TargetStatusActionSelectLineage ||
		!reflect.DeepEqual(status.CandidateLineageIDs, []string{recovery.LineageID, claimant.LineageID}) {
		t.Fatalf("STATUS = %#v, %v", status, err)
	}
}

func TestApprovedNoncanonicalSingletonIsNotRecoverable(t *testing.T) {
	repo, base, _, store, _ := approvedBaseDiffScopeRecoveryFixture(t, "noncanonical-singleton")
	receipt, err := os.ReadFile(store.ReceiptPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || status.Applicability != TargetApplicabilityUnrelated || status.LineageID != "" ||
