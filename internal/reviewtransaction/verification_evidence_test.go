...
func TestSubstantiveVerificationPayload(t *testing.T) {
	tests := []struct {
		...
		{name: "duplicate opaque token", payload: "x x", want: false},
		{name: "duplicate opaque token case-insensitive", payload: "x X", want: false},
		{name: "question-mark wrapped sentinels", payload: "PASS? FAIL?", want: false},
