...
func TestRunSDDAttemptHelpContractsCoverEveryOperation(t *testing.T) {
	tests := []struct {
		...
		{"rescope", []string{"cwd", "change", "expected-revision", "request-id", "work-unit", "evidence-goal", "max-attempts", "max-changed-lines", "reason", "actor", "objective-relation"}, []string{"explicit limit", "cannot exceed current objective", "remediation (default)", "independent"}},
