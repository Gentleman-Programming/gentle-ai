...
func TestRunSDDAttemptHelpContractsCoverEveryOperation(t *testing.T) {
	tests := []struct {
		...
		{"settle", []string{"cwd", "change", "token", "request-id", "outcome", "evidence-revision", "diagnosis", "harness-disposition", "cleanup-evidence", "process-evidence", "successor-lineage", "remediates-evidence-revision", "remediation-evidence-file"}, []string{"opaque token returned by acquire", "required for failed/passed; omit for interrupted"}},
