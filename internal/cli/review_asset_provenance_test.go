...
func TestManagedReviewerAssetProvenanceAuthorityBoundary(t *testing.T) {
	...
	decoy := filepath.Join(home, ".config", "opencode", "plugins", "reviewer-shim.ts")
	requireManagedAssetProvenanceNoError(t, os.MkdirAll(filepath.Dir(decoy), 0o755))
	requireManagedAssetProvenanceNoError(t, os.WriteFile(decoy, []byte(assets.MustRead("opencode/plugins/reviewer-shim.ts")), 0o644))
