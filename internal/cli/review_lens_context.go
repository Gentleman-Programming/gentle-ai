...
Evidence class. The class states what your proof rests on, and it decides whether anything ever checks your claim. Use "deterministic" only when the sections above prove the claim by themselves — the defect is visible in these bytes and needs no fact from outside them, because a deterministic finding is corroborated with no further review. Use "inferential" when the claim depends on anything you cannot see here: the documented behaviour of an external API, library, runtime, or service, what a caller elsewhere does, or expected production conditions. Recalled knowledge about a third party is inferential no matter how confident you are, and an inferential finding is checked by a refuter before it can block. Use "insufficient" when you could not gather enough evidence to stand behind the claim at all.
