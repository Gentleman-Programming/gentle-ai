...
const installChannelHelp = "Axiom channel: stable (default), beta, or nightly (alias for beta) — env: AXIOM_CHANNEL (fallback: GENTLE_AI_CHANNEL)"
...
FLAGS
  ...
  --scope global|workspace           Install scope (env: AXIOM_INSTALL_SCOPE, fallback: GENTLE_AI_INSTALL_SCOPE)
  --channel stable|beta|nightly      Release channel; nightly is an alias for beta (env: AXIOM_CHANNEL, fallback: GENTLE_AI_CHANNEL)
...
func ParseInstallFlags(args []string) (InstallFlags, error) {
	...
	fs.StringVar(&opts.Scope, "scope", "", "install scope: global (default) or workspace — env: AXIOM_INSTALL_SCOPE (fallback: GENTLE_AI_INSTALL_SCOPE)")
