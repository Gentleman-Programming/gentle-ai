...
func RunReview(args []string, stdout io.Writer) error {
	if len(args) == 0 || args[0] == "help" || args[0] == "-h" || args[0] == "--help" {
		_, _ = fmt.Fprintln(stdout, "Usage: gentle-ai review <advisory|capture-evidence|capture-result|capabilities|start|finalize|validate|status|repair|invalidate|abandon|recover|retry-final-verification|reclaim|inspect-authority|inspect-candidate|lens-context|dispose-result|reopen-results|schema|preserve-result|bind-sdd> [flags]\n\nOrdinary review facade; repository scope, authority, canonical artifacts, and lifecycle transitions are derived by Go. `review advisory` validates model transport output for later native admission; it never directly creates a receipt or delivery decision. Use review retry-final-verification only for a provider-proven completed failed final-verification tooling incident. Generic review recover remains unchanged. Use review repair --preflight for provider-owned classified authority repair.")
		_, _ = fmt.Fprintln(stdout, "Additive headless capabilities: gentle-ai review capture-evidence, gentle-ai review capture-result (with --preflight), gentle-ai review inspect-candidate, gentle-ai review lens-context, and gentle-ai review preserve-result.")
		return nil
	}
