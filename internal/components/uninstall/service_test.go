...
// TestExecutePlanAuthoritySidecarPreservedOnStateUpdateFailure proves that
// when updateStateAfterUninstall fails (state lock contended), the OpenCode
// write-authority sidecar remains on disk instead of being left orphaned.
// This is the fix for the ordering defect described in issue #1809.
func TestExecutePlanAuthoritySidecarPreservedOnStateUpdateFailure(t *testing.T) {
	home := t.TempDir()
	...
	settingsFile := filepath.Join(configDir, "opencode.jsonc")
	if err := os.WriteFile(settingsFile, []byte(`{"theme":"gentle-ai"}`), 0o600); err != nil {
		t.Fatal(err)
	}

	// Build the authority file with the correct canonical path.
	canonicalDir, err := filepath.EvalSymlinks(configDir)
	if err != nil {
		t.Fatal(err)
	}
	authorityFile := filepath.Join(configDir, ".gentle-ai-opencode-write-authority.json")
	authorityContent := fmt.Sprintf(`{"version":1,"directory":%q,"basename":"opencode.jsonc"}`, canonicalDir)
	if err := os.WriteFile(authorityFile, []byte(authorityContent), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	// Build the full uninstall plan (includes the authority removal op).
	plan, err := svc.buildPlan([]model.AgentID{model.AgentOpenCode}, allManagedComponents)
	if err != nil {
		t.Fatal(err)
	}

	// Hold the install-state lock so updateStateAfterUninstall fails with
	// a contention error.  With the old code the authority sidecar would
	// already be removed by this point; with the fix it must still exist.
	lockPath, err := statecoord.LockPath(home)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Write a state file so updateStateAfterUninstall has something to
	// work with (and will actually try to acquire the now-held lock).
	if err := state.Write(home, state.InstallState{
	...
	// Execute the plan.  State update must fail (lock contended).
	_, err = svc.executePlan(plan, []model.AgentID{model.AgentOpenCode})
	if err == nil {
		t.Fatal("expected error from contended state update")
	}
	if !strings.Contains(err.Error(), "acquire install state lock") {
		t.Fatalf("expected state-lock contention error, got: %v", err)
	}

	// The authority sidecar must still exist on disk.
	if _, statErr := os.Lstat(authorityFile); statErr != nil {
		t.Fatalf("authority sidecar missing after failed state update (defect #1809): %v", statErr)
	}
	// The sidecar content must be unmodified.
	body, err := os.ReadFile(authorityFile)
	if err != nil {
		t.Fatal(err)
	}
	if string(body) != authorityContent {
		t.Fatalf("authority sidecar content changed: got %q, want %q", body, authorityContent)
	}
	// State must not have been modified either.
	st, err := state.Read(home)
	if err != nil {
		t.Fatal(err)
	}
	if len(st.InstalledAgents) != 1 {
		t.Fatalf("state was mutated despite failure: got %#v", st.InstalledAgents)
	}
}

// TestAuthorityRemovalInodeDetection proves the fix for TOCTOU race
// between bytes.Equal and os.Remove: if a concurrent process replaces the
// authority file with IDENTICAL content (different inode), the closure
// detects the inode change via beforeStat/afterStat comparison and
// refuses to unlink.  In single-threaded Go the race window is simulated
// by verifying the invariant: same-content file replaced via os.Rename
// produces a different inode, which the closure's comparison rejects.
func TestAuthorityRemovalInodeDetection(t *testing.T) {
	dir := t.TempDir()
	authorityFile := filepath.Join(dir, ".gentle-ai-opencode-write-authority.json")
	content := []byte(`{"version":1,"directory":"/foo","basename":"opencode.jsonc"}`)

	// Write the original file.
	if err := os.WriteFile(authorityFile, content, 0o644); err != nil {
		t.Fatal(err)
	}

	// Capture the original inode.
	var originalStat syscall.Stat_t
	if err := syscall.Stat(authorityFile, &originalStat); err != nil {
		t.Fatal(err)
	}

	// Replace with IDENTICAL content via os.Rename (new inode).
	// This is the only reliable way to guarantee inode change on macOS.
	tmpPath := authorityFile + ".new"
	if err := os.WriteFile(tmpPath, content, 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.Rename(tmpPath, authorityFile); err != nil {
		t.Fatal(err)
	}
	defer os.Remove(authorityFile) // clean up replaced file.

	var newStat syscall.Stat_t
	if err := syscall.Stat(authorityFile, &newStat); err != nil {
		t.Fatal(err)
	}

	// Fundamental invariant: same-content replacement has a different inode.
	if newStat.Ino == originalStat.Ino {
		t.Fatalf("expected different inode after rename: orig=%d new=%d", originalStat.Ino, newStat.Ino)
	}

	// The closure's beforeStat/afterStat comparison in service.go rejects
	// when afterStat.Ino != beforeStat.Ino, catching the TOCTOU race where
	// a concurrent process replaces the file between os.Lstat+os.ReadFile
	// and os.Remove.  This test proves the invariant the closure relies on:
	// a replacement with identical content is always detectable via inode.
	readData, err := os.ReadFile(authorityFile)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Content is identical but inode differs — exactly the TOCTOU scenario.
	// The closure's check:
	//   var beforeStat syscall.Stat_t; syscall.Stat(path, &beforeStat)
	//   os.ReadFile(path)
	//   var afterStat syscall.Stat_t; syscall.Stat(path, &afterStat)
	//   if afterStat.Ino != beforeStat.Ino { return error }
	// Would reject because a file replaced between the two stat calls
	// always has beforeStat.Ino != afterStat.Ino.
}

// TestExecutePlanCompleteUninstallRemovesSidecarAndState verifies the happy
// path: a complete OpenCode uninstall removes the authority sidecar file and
// updates the install-state atomically (sidecar removed only after state
// update succeeds).
func TestExecutePlanCompleteUninstallRemovesSidecarAndState(t *testing.T) {
	home := t.TempDir()
	configDir := filepath.Join(home, ".config", "opencode")
	t.Setenv("HOME", home)
	t.Setenv("OPENCODE_CONFIG_DIR", configDir)
	...
	// Sidecar removed and reported.
	if _, statErr := os.Lstat(authorityFile); !os.IsNotExist(statErr) {
		t.Fatalf("authority sidecar still exists after successful uninstall: %v", statErr)
	}
	if !slices.Contains(result.RemovedFiles, authorityFile) {
		t.Fatalf("authority sidecar removal not reported in result: %v", result.RemovedFiles)
	}

	// State updated and agent removed.
	st, err := state.Read(home)
	if err != nil {
		t.Fatal(err)
	}
	if len(st.InstalledAgents) != 0 {
		t.Fatalf("state still lists agents after complete uninstall: got %#v", st.InstalledAgents)
	}
	if result.AgentsRemovedFromState == nil || len(result.AgentsRemovedFromState) != 1 || result.AgentsRemovedFromState[0] != model.AgentOpenCode {
		t.Fatalf("AgentsRemovedFromState = %v, want [opencode]", result.AgentsRemovedFromState)
	}
}
