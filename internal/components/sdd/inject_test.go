	...
	for _, tc := range tests {
		t.Run(string(tc.agent), func(t *testing.T) {
			...
			for _, required := range []string{
				...
				"confirm the newly-created issue identity/URL",
				"without requesting, supplying, or applying any label during creation",
				"The repository-side `issues: opened` workflow is the only writer of `gentle-report`",
				"Do not create, comment, update, or label that new issue",
				"If creation definitively fails or lacks permission, STOP with all consumer state preserved",
				"permit only a read-only GitHub lookup narrowly scoped to resolving the exact issue identity from that original creation attempt",
