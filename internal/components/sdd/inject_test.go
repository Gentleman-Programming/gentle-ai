...
func TestPropagateTopLevelPermissions(t *testing.T) {
	tests := []struct{ name, input, want string }{
		...
		{"missing permission receives top-level deny", `{"permission":{"read":{"**/.env":"deny"}},"agent":{"gentle-orchestrator":{},"sdd-orchestrator-x":{}}}`, `{"read":{"**/.env":"deny"}}`},
		{"empty permission receives top-level deny", `{"permission":{"read":{"**/.env":"deny"}},"agent":{"gentle-orchestrator":{"permission":{}},"sdd-orchestrator-x":{"permission":{}}}}`, `{"read":{"**/.env":"deny"}}`},
		{"existing task permission survives read propagation", `{"permission":{"read":{"*":"allow","**/.env":"deny"}},"agent":{"gentle-orchestrator":{"permission":{"task":{"*":"deny","sdd-apply":"allow"}}},"sdd-orchestrator-x":{"permission":{"task":{"*":"deny","sdd-apply":"allow"}}}}}`, `{"read":{"*":"allow","**/.env":"deny"},"task":{"*":"deny","sdd-apply":"allow"}}`},
