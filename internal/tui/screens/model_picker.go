...
func NewModelPickerState(cachePath string, settingsPath string) ModelPickerState {
	...
	if len(configProviders) == 0 && configErr == nil && settingsPath != "" {
		if providersFromSettings, err := opencode.LoadConfigProviders(settingsPath); err != nil {
			configErr = err
		} else {
			configProviders = providersFromSettings
		}
	}
	...
	if configErr != nil {
		configWarning = appendConfigWarning(configWarning, fmt.Sprintf("Could not load custom providers from OpenCode config: %v", configErr))
	}
