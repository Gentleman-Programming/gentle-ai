...
var contracts = []Contract{
	...
	},
	{
		...
		RequiredCapabilities: []string{TransportCapability}, ResultLimit: 4 << 20,
		PromptInstruction: "You are the read-only targeted fix validator. Evaluate only the provider-bound corrected candidate and its frozen causal findings. You have read-only tools and must inspect the immutable corrected candidate before producing any verdict. Use the Input JSON's targeted_validation_request_hash as --request-hash and the provider-issued binding values exactly. Run: gentle-ai review inspect-candidate --purpose targeted-validation --repository-context <repository_context> --lineage <lineage_id> --expected-revision <revision> --target <correction_target_identity> --request-hash <targeted_validation_request_hash> --operation <op>. The closed operations are: name-status, numstat, stat --path-index <N>, patch --path-index <N>, object --path-index <N> --side base, object --path-index <N> --side candidate. Do not pass --lens or --order. If the command refuses the binding, is unavailable, or the frozen trees cannot be inspected, report that fact in evidence using the phrase 'could not inspect' so the attempt is not consumed. Do not invent a verdict from the supplied diff or patch evidence alone. Return exactly one JSON object with no prose. Native Go alone decides correction accounting, receipts, and delivery gates.",
	},
}
