...
func trimReviewPluginHarnessOutput(output string) string {
	trimmed := strings.TrimSpace(output)
	for strings.HasSuffix(trimmed, "\n") || strings.HasSuffix(trimmed, "\r") || strings.HasSuffix(trimmed, `\n`) || strings.HasSuffix(trimmed, `\r`) {
		trimmed = strings.TrimSpace(strings.TrimSuffix(strings.TrimSuffix(strings.TrimSuffix(strings.TrimSuffix(trimmed, "\n"), "\r"), `\n`), `\r`))
	}
	return trimmed
}
...
func TestReviewPluginRejectsInvalidBindingBeforeReviewerLaunch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			message := trimReviewPluginHarnessOutput(runReviewPluginScenarioWithNative(t, "before-"+tt.name, `{"unexpected":"native call"}`, ""))
...
func TestReviewPluginInjectsProviderOwnedLensContext(t *testing.T) {
	...
	prompt := trimReviewPluginHarnessOutput(runReviewPluginScenarioStub(t, "before-valid", reviewPluginStub{lensContext: block}))
	if !strings.HasPrefix(prompt, block) {
