...
- Every backticked path on a checkbox line counts as an edit target for edit authority. A path the task only reads (a sibling repository's release artifact, a fixture, a reference file) MUST carry `(read-only)` on the same line, for example `- [ ] 1.2 Compare against \`../service-b/dist/release-1.0.tgz\` (read-only)`; without the marker, a path outside the authorized edit roots blocks apply with `blocked(edit_authority_missing)`.
