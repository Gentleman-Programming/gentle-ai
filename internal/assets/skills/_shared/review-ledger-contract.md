...
| `review_decision_required` | B paused with unresolved evidence: a human decides. `gentle-ai review decide --decision continue` resumes B; `--decision stop` closes it escalated; STATUS carries both runnable invocations with the current revision. |
