...
3. **Stay bound.** Every later STATUS and collection call for that transaction passes the exact captured lineage, revision, and target tokens. Route only from that transaction's returned `next_transition`; never infer a command from prose, ambient state, a gate, or a stale reply. Do not restart an acknowledged terminal lineage or perform ambient recovery. For `execute`, run the exact operation and ordered arguments. For `collect`, satisfy only the named inputs and their exact capture operations, then ask STATUS again with the same binding. For `stop`, run no lifecycle operation.

4. **Acknowledge exactly.** Native Go owns frozen lenses, provider context and admission, refutation, one bounded correction, repository evidence, targeted validation, and approved closure. A final approved capture commits one pending acknowledgement token and returns `review.acknowledge-approved`; restarted STATUS returns the same operation, ordered arguments, token, and live revision. Only that exact invocation consumes the token while retaining the approved compact authority and its candidate binding. Wrong, stale, or replayed acknowledgement refuses without creating a receipt, tombstone, witness, mirror, or sidecar. Unrelated transactions survive unchanged.
...
- After B is selected, retain canonical B as the lifecycle working root from selectorless STATUS through consent, collection, correction, targeted validation, acknowledgement, and exact delivery validation. Do not fall back to A.
...
- The same lineage text in A and B is independent. Approval awaits acknowledgement in B; exact acknowledgement retains B's approved authority, and A remains untouched.
...
| 05 | STATUS restart + exact acknowledgement | replayed operation/token/revision; exact acknowledgement consumes only the token and retains approval |
| 06 | terminal lifecycle stop | explicit-lineage pre-commit may validate the exact staged candidate; other delivery follows ordinary repository policy |

<!-- authority-first-terminal-procedure:end -->

### Continue after a stop reason code

A `stop` ends its transition and never approves delivery by itself. Complete atomic inventory: B stays target root; only an acknowledged exact-lineage pre-commit gate can become managed. `D` means `gentle-ai review mode disable --scope clone --cwd <B>`; B ordinary policy decides delivery. `S` means re-query the exact captured target-root STATUS command with lineage and target.
...
RDD uses only the selected local Git worktree and common directory; no remote or upstream. Disabled gates return `disabled/unmanaged`. Enabled gates without an exact lineage, or gates other than `pre-commit`, return `invalidated/unmanaged`.

An enabled `pre-commit --lineage <id>` reads retained compact authority and the local staged candidate. It allows only approved, acknowledged authority with exact base tree, candidate tree, and paths digest; pending approval, ambiguity, or drift denies. No receipt mirror or parallel authority is created.

The review lifecycle stops at acknowledgement but retains `approved` authority. A gate allow authorizes only the requested pre-commit continuation; commit, push, PR, merge, and release remain human-owned.
