...
- Detection is conservative prose inspection: backticked path-like tokens inside markdown checkbox lines that resolve to a path outside the authorized roots. A different repository is named by its Git root; a same-repository target is narrowed to its containing edit root; a directory in no Git repository is named as itself. A checkbox line carrying `(read-only)` (case-insensitive; `read-only:` as the line prefix is also accepted) names no edit target. A context reference without that marker can raise a false consent question; the consequence is a question, never silent authority.
