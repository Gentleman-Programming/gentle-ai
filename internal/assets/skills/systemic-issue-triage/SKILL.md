...
- If a fix crosses a systemic-cluster boundary another issue already tracks, STOP and write one unified design for the cluster (owner, error precedence, migration table per call site) before any PR. Duplicating a sibling issue's scope is a defect.
