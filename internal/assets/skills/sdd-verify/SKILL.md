...
- Build the complete report as exact candidate bytes before any OpenSpec or Engram write. For a whole-change report, count retrieved specifications and run `gentle-ai sdd-verify-validate --input <path|-> --requirements <n> --scenarios <n>` with those required authoritative totals. For a provider-owned slice, run `gentle-ai sdd-verify-validate --input <path|-> --cwd <repo> --change <name> --scope slice --slice-id <runtime-objective-id>`, declare matching report envelope fields, use only the runtime objective's immutable assignments, and never provide caller totals or spec-wide totals. If the validator is unavailable or denies admission, make zero writes and leave the prior report untouched; otherwise persist the same bytes, including a valid `fail`. A valid `fail` scoped report creates no SliceProof or runtime write and keeps advancement blocked; only `pass` and `pass_with_warnings` can create a SliceProof.
