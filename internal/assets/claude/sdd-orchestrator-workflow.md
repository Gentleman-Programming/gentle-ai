...
3. After a failed or passed run, call `gentle-ai sdd-attempt settle --cwd <repo> --change <change> --token <token> --request-id <settle-id> --outcome <passed|failed> --evidence-revision <sha256> --diagnosis "<proven-diagnosis>" --harness-disposition <reused|invalidated> --cleanup-evidence "<evidence>" --process-evidence "<evidence>"`. After an interrupted run, pass `--outcome interrupted` and omit `--evidence-revision`. When the acquire carried `--remediates-evidence-revision <sha256>`, settle with the same `--remediates-evidence-revision <sha256>`. Use a `<settle-id>` distinct from the acquire operation's request ID; a successful settlement commits its request record so any replay with that ID must be an exact idempotent replay, whereas a refused attempt records no request record and permits a corrected same-ID retry. For `blocked(undeclared_untracked)`, retry compact `sdd-attempt settle` with the same `<settle-id>` even though the declaration changes: use the fresh `expected_untracked_inventory` digest from the refusal, retain the intended-untracked selection floor, and never route through acquire or Review STATUS. Settle derives native binding and remediation inputs itself.
