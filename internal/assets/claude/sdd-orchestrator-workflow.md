...
3. After a failed or passed run, call `gentle-ai sdd-attempt settle --cwd <repo> --change <change> --token <token> --request-id <settle-id> --outcome <passed|failed> --evidence-revision <sha256> --diagnosis "<proven-diagnosis>" --harness-disposition <reused|invalidated> --cleanup-evidence "<evidence>" --process-evidence "<evidence>"`. After an interrupted run, pass `--outcome interrupted` and omit `--evidence-revision`. When the acquire carried `--remediates-evidence-revision <sha256>`, settle with the same `--remediates-evidence-revision <sha256>`. Use a `<settle-id>` distinct from the acquire operation's request ID; reuse each operation's own ID only for its idempotent replay. For `blocked(undeclared_untracked)`, retry compact `sdd-attempt settle` with the same `<settle-id>`, using the fresh `expected_untracked_inventory` digest from the refusal, retaining the intended-untracked selection floor, and never routing through acquire or Review STATUS. Settle derives native binding and remediation inputs itself.
