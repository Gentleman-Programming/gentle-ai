...
#### Strictly-Closed Single-Select Domain (MANDATORY)

When a single-select blocking choice envelope is strictly closed (discrete alternatives with label, description, effect, and no free-text / multi-select admissibility), the classified native question UI MUST be used whenever the complete envelope is exactly representable in one grouped interaction. Free text MUST NOT be admitted on a strictly-closed single-select; this is the strictest sub-case of the answer-validation clause above and overrides any permissive free-text path the installed runtime surfaces by default.

- Answer normalization: accept equivalent semantic selections against the canonical internal token. The validator MUST accept case-insensitive matches, locale-agnostic whitespace trim, and ordinal references (`la 1`, `the first`, `primera`, `opción 2`) that map to the option's index in the presented group. The validator MUST reject only inputs that match no presented option after normalization.
- Strict-closed fallback: when the classified interactive runtime is available and can render the options but cannot guarantee the closed-domain answer gate, the orchestrator MUST surface ONE actionable compatibility limitation naming the strict-closed gap and MUST NOT repeat the exact-token prompt. This is a fifth OR-additive fallback trigger, independent of the four enumerated in the fallback clause above.
- Lossless invariant: labels, descriptions, effects, and option order are preserved as in the original envelope. When the strictly-closed envelope extends the SDD edit-authority consent relay (`granted` / `declined`) or the provider-defect handoff (`report_and_continue` / `continue_without_reporting` / `stop_here`), the existing token discipline and exact-machine-token rules still apply.
