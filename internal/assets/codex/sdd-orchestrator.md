...
When review-derived or verification-derived findings require `sdd-apply` remediation, launch a fresh provider executor/model context through the Codex phase prompt. If the provider exposes resumable task identity, do not forward the prior apply `task_id`.
