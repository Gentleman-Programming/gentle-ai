...
When a single-select blocking choice envelope is strictly closed (discrete alternatives with label, description, effect, and no free-text / multi-select admissibility), this variant cannot guarantee a classified strictly-closed native UI; the actionable compatibility limitation below is the strict-closed path here. Free text MUST NOT be admitted on a strictly-closed single-select, and the answer normalization and lossless invariant rules apply identically.
