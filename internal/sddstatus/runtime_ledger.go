...
const (
	...
	ResetDispositionFailedEvidenceRemediation = "failed-evidence-remediation"
...
func normalizeResetObjectiveRequest(request ResetObjectiveRequest) (ResetObjectiveRequest, error) {
	...
	for _, field := range []struct {
		...
		{name: "remediates_evidence_revision", present: request.RemediatesEvidenceRevision != ""},
		{name: "work_unit", present: request.WorkUnit != ""},
		{name: "evidence_goal", present: request.EvidenceGoal != ""},
		{name: "max_changed_lines", present: request.MaxChangedLines != 0},
