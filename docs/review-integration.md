...
A compact-v2 recovery successor whose `gentle-ai.review-recovery-authorization/v1` binding names a `target_identity` it never persisted is not a `repair_authority` candidate: the authorized identity has no persisted candidate tree, so it can never be re-derived into a reconcilable class, and `review repair` correctly reports it as unsupported. That shape is served by the dedicated `gentle-ai review repair-recovery-binding` verb. Its `--preflight` response carries schema `gentle-ai.review-recovery-binding-repair/v1` with per-edge candidates — never one per repository — each supplying provider-owned class, cause, disposition, opaque repository binding, both lineage IDs, both expected revisions, the persisted and authorized target identities, and the recorded authorization digest, plus a `blocked` note naming the lineage to disposition first when the leaf-first ordering fence would refuse that edge right now. The maintainer then provides `actor`, `reason`, and the exact fourteen-line LF-only authorization. No response emits a repository path, quarantine record, or ready-made authorization string.
