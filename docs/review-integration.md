...
A captured severe candidate-causal finding whose `evidence_class` is `inferential` cannot be resolved by FINALIZE alone: it requires one refuter outcome. STATUS therefore answers `collect` with reason `refuter_outcomes_required` before it ever offers finalization. That transition carries exactly one `external.run_refuter` input whose `refuter_claims` array is the complete merged batch — every pending claim with its finding ID, frozen snapshot identity, and classification proof. The refuter budget is review-level and structural: run ONE read-only refuter over the whole batch and return one verdict per claim, never one refuter per finding. Submit the batch with `gentle-ai review finalize --lineage <id> --captured-results --refuter <path>`; the input carries no submission descriptor because `--refuter` is an existing FINALIZE flag rather than a provider-bound value slot. A `deterministic` finding is corroborated with no refuter, and an `insufficient` one escalates, so neither ever produces this transition.
