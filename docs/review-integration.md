...
Claude Code and OpenCode provide the enforced fresh reviewer-execution boundary: Claude reviewers are tool-free fresh agents with prompt-carried native evidence, and OpenCode provider injection replaces the reviewer task with bound native evidence under its required host isolation controls. Codex and Kilo receive `immutable_review_transport_unsupported` before repository, target, authority, collection, or process work; that typed result names the clone-scoped review-mode exit and the supported runtimes. Normal SDD and the manual compatibility path remain available; no provider or model is substituted.
