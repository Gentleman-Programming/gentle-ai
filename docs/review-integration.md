...
Before a correction forecast, `correction_plan_required` collects the bounded correction plan. Once a positive forecast is persisted, `correction_candidate_required` collects the external bounded candidate edit. Its `correction_candidate` input has schema `gentle-ai.review-correction-plan-request/v1`, capture operation `external.apply_correction`, and the immutable lineage, revision, and frozen-target arguments. The attached provider-derived request hash binds the current lineage, authority revision, frozen target, correction budget, canonical `fix_finding_ids`, and each accepted finding's location, claim, proof references, classification evidence, evidence class, and causal disposition. Reading either transition does not consume an attempt or correction budget. Consumers plan and edit only from this accepted set, never from raw reviewer output.
...
Execute the targeted request unchanged. Its provider-derived hash binds the lineage, expected authority revision, original target, exact frozen finding IDs, projection, corrected candidate tree and identity, and the exact canonical correction-path subset plus its digest. FINALIZE accepts the correction through one atomic state transition only when the targeted validation and passed repository record bind the same authority revision, candidate identity, paths, and ledger IDs. Until the candidate materially changes, `correction_candidate_required` remains the only route and no targeted-validation request is issued. Consumers must not invent a validator request or another correction forecast. An ordinary lineage admits exactly one changed-target correction attempt, even when its measured delta is zero. It never admits a zero-edit correction or second fix transition.
