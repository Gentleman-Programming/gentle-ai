...
Protocol v1.4 advertises `gentle-ai.review-integration.capabilities/v1.4` and adds `one_shot_final_verification_retry` and operation `review.retry_final_verification`. This is a dedicated provider-owned retry for one exact completed failed final-verification tooling incident; it does not relax generic recovery.
...
When the action is `recover`, negotiated status also returns the exact generic recovery disposition: `scope_changed`, `escalated`, or `invalidated`. A materially changed escalated candidate exposes only generic `review.recover`. An unchanged escalated candidate normally exposes only `stop`; eligible final-verification retry is exposed only through the opaque consent relay described below. A consumer MUST NOT substitute a different disposition or route the dedicated class through generic `review recover`.
...
| `final_verification_retry_context_unavailable` | Re-run `gentle-ai review status --cwd <repo> --contract gentle-ai.review-integration/v2 --agent <agent> --next-transition` so the provider can issue the opaque retry context. |
| `legacy_hash_only_authority` | Terminal for this historical hash-only review record. Produce a new candidate target identity, then run `gentle-ai review start --cwd <repo>` to begin a new review, or run `gentle-ai review mode disable --scope clone --cwd <repo>` to deliver under ordinary policy instead. Never retry, import, or repair the historical record. |
...
`gentle-ai review retry-final-verification` is provider-only and one-shot across the entire ancestry. Only a provider-verified `procedural_tooling_failure` can reach this route. A v2 STATUS response that proves eligibility returns `next_transition.collect.consent` with schema `gentle-ai.review-final-verification-retry-consent/v1`, two choices, and one opaque runnable invocation per choice.

Quick path: query STATUS with `--next-transition`; relay the complete consent envelope as a blocking prompt; map the selected human label once to its original `granted` or `declined` token; execute the exact returned invocation once; then query STATUS again. The invocation owns the repository context and all retry bindings. Consumers MUST NOT build an incident, authorization, successor identity, or substitute command. A declined invocation changes no authority. A granted invocation creates at most one provider-derived validating successor; its exact replay converges on that successor. Capture new final evidence against the successor through its returned STATUS transition, then use normal FINALIZE. Success approves; another failure escalates permanently.
