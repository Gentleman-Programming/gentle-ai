...
Claude Code is the only supported v2.1 immutable receipt-review transport. OpenCode and Codex are eligible but transport-disabled; Pi, Kilo, unknown, alias, and casing variants are ineligible. STATUS and START stop with `immutable_review_transport_unsupported` before repository, target, authority, collection, or process work when the declared runtime lacks that transport.

Every runtime's generated route declares its own identity, never a borrowed one: the negotiated STATUS command each adapter receives names that adapter's `--agent` token. A route that carries no `--agent` token, or more than one, is a malformed or stale route rather than a transport claim; STATUS and START stop with `runtime_identity_unresolved` and `next_action: correct_request`, and the additive `cause` names `gentle-ai sync` as the way to regenerate the route.
