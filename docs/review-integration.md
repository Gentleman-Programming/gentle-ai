...
`gentle-ai review advisory` is the provider-owned prompt/text seam for the two advertised review runtimes, Claude Code and OpenCode. Codex is wired but unadvertised: its shared `CodexAdapter` transport was proven organically (a real `codex exec` process, launched in an empty scratch directory it creates and deletes itself, received only the canonical prompt and its raw output reached native admission) and remains callable as a transport, but Codex no longer advertises either review contract on any surface, so a session that requests it is refused before any repository, target, authority, collection, or process work. Any runtime name outside the advertised set (`claude-code`, `opencode`) still gets `PromptFor`'s typed unavailable error.
...
Claude Code and OpenCode provide the enforced fresh reviewer-execution boundary through the one shared advisory contract: Claude reviewers are tool-free fresh agents with prompt-carried native evidence, and OpenCode provider injection replaces the reviewer task with bound native evidence from an ordinary already-running session (no restart, child process, special user-visible session, or `OPENCODE_DISABLE_PROJECT_CONFIG` / `OPENCODE_DISABLE_EXTERNAL_SKILLS` variable). Codex is wired but unadvertised: its shared advisory `CodexAdapter` still launches a brand-new `codex exec` process in an empty scratch directory it creates and deletes itself, receiving only the canonical `review advisory prompt` output, but Codex advertises neither review contract and receives `immutable_review_transport_unsupported` before repository, target, authority, collection, or process work; that typed result names the clone-scoped review-mode exit and the supported runtimes. Normal SDD and the manual compatibility path remain available; no provider or model is substituted.
