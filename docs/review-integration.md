...
| `rdd_disabled` | Run the exact source-scoped `gentle-ai review mode enable` command rendered with this STATUS result, then re-run its exact repository-bound STATUS command. Receipt-driven development is opt-in, so the most common case is a repository where nobody enabled it: the deciding source is `default` and the rendered command is `--scope=global`, the only scope that can turn review on. |
