...
Claude Code, OpenCode, and Codex provide the enforced reviewer-execution boundary through one shared provider contract: Claude reviewers are tool-free fresh agents with prompt-carried native evidence; OpenCode relays a task through its ordinary running session with no restart, isolation session, network listener, temporary candidate bytes, child OpenCode process, or `OPENCODE_DISABLE_*` variables; and Codex launches its provider-bound subprocess. Kilo receives `immutable_review_transport_unsupported` before repository, target, authority, collection, or process work; that typed result names the clone-scoped review-mode exit and the supported runtimes. Normal SDD and the manual compatibility path remain available; no provider or model is substituted.
...
| `targeted_validation_inconclusive` | The targeted validator could not inspect the immutable corrected candidate, so no verdict was produced and the correction attempt was not consumed. Restore validator access to the frozen trees and capture the same validation again. |
