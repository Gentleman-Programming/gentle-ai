...
- Git, an effective Go 1.22+ toolchain (the installed binary may be older when automatic toolchain selection provides it), and `sha256sum`. Your existing `HOME`, authentication, and client configuration are used unchanged.
