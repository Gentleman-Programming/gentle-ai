...
When managed activation is enabled, Gentle AI owns launchers under `~/.gentle-ai/bin/` (`opencode` on POSIX, `opencode.cmd` and `opencode.ps1` on Windows). On POSIX, it adds a marked, removable PATH block to `.zprofile` for zsh login shells, or to an existing `.bash_profile` for bash login shells. The launcher sets `OPENCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true` only when the variable is absent, so an explicit `OPENCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=false` remains foreground. Start a new supported login shell before running `opencode`; until the bare command resolves to the managed launcher, `doctor` reports managed activation as failed and the overall status as unhealthy.
