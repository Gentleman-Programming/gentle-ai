...
> - **Fase 4: Evolución Arquitectónica, Flujo Orgánico (ODD) y Absorción Upstream v3:** ⏳ En progreso (2/4 Incrementos Archivados)  
> **Total de Incrementos Archivados:** 19 de 21 (INC-01 a INC-19)  
> **Total de Incrementos En Progreso:** 2 (INC-20, INC-21)  
...
| **INC-21** | `inc-21-upfront-flow-governance` | ⏳ En progreso | Orquestador / Gobernanza SDD | Determinación temprana de carril ODD/SDD con pregunta bloqueante (REQ-21.1-21.3); cuestionario de pre-vuelo que sella modalidad de avance, política de relevos y roster de roles antes de `proposal.md` (`axiom sdd kickoff seal/show`, paquete `internal/kickoff`), con `fullstack` como identidad reservada obligatoria sin subdivisión (REQ-21.5/21.6, D-06/D-07); cuatro compuertas de revisión por bloque (`spec`, `design`, `tasks`, `apply` por rol) evaluadas solo en modalidad con paradas, con reapertura por digest de artefacto y `approved` terminal (`axiom sdd gate record/show`, REQ-21.7-21.12); aviso formal y relevo de integración automático al cerrar el último rol activo (REQ-21.13/21.14); precondición de integración o despliegue verificable localmente (`git merge-base --is-ancestor`, sin operaciones remotas) antes de permitir `archive` (REQ-21.16); e inmutabilidad post-archive con gestión exclusiva vía bug o nuevo incremento (REQ-21.18). Frontera normativa explícita frente a *receipt-driven development*: las compuertas SDD nunca son un recibo ni autorizan entrega por sí mismas. |
