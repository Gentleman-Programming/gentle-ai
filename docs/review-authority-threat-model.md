...
## Drifted Recovery Binding Quarantine

`gentle-ai review repair-recovery-binding` is a separate narrow maintenance operation for one compact-v2 recovery successor whose recorded `gentle-ai.review-recovery-authorization/v1` binding names a `target_identity` the successor never persisted. It is a quarantine, never validation, rewrite, migration, or rebinding.

The authorized identity has no persisted candidate tree, so the edge can never be re-derived into a reconcilable class; generic reconciliation stays closed to it deliberately. The classifier therefore keys on the fully persisted internal disagreement between the successor's `initial_snapshot.identity` and its binding's `target_identity`, and admits the shape only when that identity is the sole deviation — the binding rebuilt from the persisted predecessor record plus persisted provenance must otherwise match the recorded string byte for byte, and rebinding to the persisted identity must leave a completely valid edge. Retry edges, accounting-only escalations, valid unchanged-target recoveries, and pre-contract authorizations are each refused.

Run `gentle-ai review repair-recovery-binding --preflight` first. It lists every admissible edge as its own candidate — never one per repository — ordered leaf-first, descendants before ancestors, with both lineage IDs, both expected revisions, and every drift value the authorization binds. A repository holding several drifted edges repairs them sequentially, and a candidate the ordering fence would refuse right now is listed with the lineage to disposition first rather than hidden. Both ordering rules are strictly ordered and never mutual, so a predecessor forking into two drifted successors cannot leave each sibling refusing the other.

The only accepted disposition is `quarantine-drifted-recovery-binding` for class `compact_v2_recovery_authorization_target_drift` and cause `recovery_authorization_target_identity_never_persisted`. The exact LF-only fourteen-line authorization order is `gentle-ai.review-repair-authorization/v1`, `repository`, `class`, `cause`, `disposition`, `predecessor_lineage`, `predecessor_revision`, `successor_lineage`, `successor_revision`, `persisted_target_identity`, `authorized_target_identity`, `recorded_authorization_sha256`, `actor`, `reason`. Public preflight and execution results never echo the completed authorization, private paths, or quarantine records.

Execution re-inspects the complete inventory under the same maintenance and store boundary before moving anything, requires both expected revisions to match under exact CAS, moves the whole successor entry into audited quarantine, and reads the record back. It never deletes or rewrites authority, and the predecessor stays untouched. A concurrent change fails exact CAS without mutation; an exact committed retry converges on the same record without moving anything twice. Replay discovery matches on the record's own lineage rather than a quarantine directory prefix, so repairing one lineage cannot dead-end a hyphen-sharing sibling.
