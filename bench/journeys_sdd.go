...
func sddUnmanagedSecondCorrectionRejected(r *journeyRun) error {
	...
	if refusal := observation.Stderr + observation.Stdout; !strings.Contains(refusal, "SDD runtime request identifier was reused with different inputs") {
		return fmt.Errorf("second correction was refused for an unexpected reason: %s", firstLine(refusal))
	}
	return nil
}
