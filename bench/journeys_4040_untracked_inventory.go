...
// untrackedRecoveryLoopRetryHint is the stable hint the refusals name.
const untrackedRecoveryLoopRetryHint = "retry the same settlement"
...
func driveUntrackedInventoryRecoveryLoop(r *journeyRun) error {
	...
	if !strings.Contains(undeclared.Stderr, untrackedRecoveryLoopRetryHint) ||
		!strings.Contains(undeclared.Stderr, "--expected-untracked-inventory=") {
		return fmt.Errorf("#4040 undeclared finish refusal omitted the retry hint or exact rerun: %s", firstLine(undeclared.Stderr))
	}
	...
	if !strings.Contains(stale.Stderr, untrackedRecoveryLoopRetryHint) ||
	...
	if deleted.ExitCode == 0 ||
		!strings.Contains(deleted.Stderr, "--expected-untracked-inventory="+refreshed.EligibleUntrackedInventory) ||
		!strings.Contains(deleted.Stderr, untrackedRecoveryLoopRetryHint) {
		return fmt.Errorf("#4040 deleted-file selection omitted the current inventory or retry hint: %s", firstLine(deleted.Stderr))
	}
	if status, err = readRuntimeStatus(r); err != nil || status.ActiveAttempt == nil {
