...
[[ "$GITHUB_REPOSITORY" == "IGutierrezZ/axiom" ]] || die "unexpected repository $GITHUB_REPOSITORY"
...
if [[ -z "$stable_ref" ]]; then
  ...
  if [[ -z "$stable_release" ]]; then
    ...
    if jq -e '.draft == true and (.assets | length) == 0' <<<"$release" >/dev/null; then
      ...
      diff -u <(printf '%s\n' "axiom_${version}_darwin_amd64.tar.gz" "axiom_${version}_darwin_arm64.tar.gz" "axiom_${version}_linux_amd64.tar.gz" "axiom_${version}_linux_arm64.tar.gz" "gentle-ai-review-provider-contract-${provider_contract_semver}.tar.gz" "gentle-ai-release-provenance-v1.tar.gz" checksums.txt checksums.txt.minisig | LC_ALL=C sort) <(jq -r '.assets[].name' <<<"$release" | LC_ALL=C sort); then
